...
//		stream.awaitDrained();
	...
	public static final class ReceiverBenchmark extends SuppliedBenchmark {
		public ReceiverBenchmark() {
			super("Receiver", ReceiverKernel.class, new Dataset("src/receiver/channel_out.out",
					(Input)Input.fromBinaryFile(Paths.get("src/receiver/channel_out.out"), Float.class, ByteOrder.BIG_ENDIAN)
//					, (Supplier)Suppliers.ofInstance((Input)Input.fromBinaryFile(Paths.get("/home/jbosboom/streamit/streams/apps/benchmarks/asplos06/fft/streamit/FFT5.out"), Float.class, ByteOrder.LITTLE_ENDIAN))
	...
	public static final class ReceiverKernel extends Pipeline<Float, Float> {
		...
		public ReceiverKernel() {
			this.add(new DeMapper(),new Equalizer(),new Demodulator(),new TurboDecoder(),new Printer());
		}
